...
def test_legal_play_color_match():
    ...
    game = create_game_with_state(
        H_1=[(RED, 5), (YELLOW, 3)], H_2=[], top_card=(RED, 7), deck=[]
    )

    assert game.is_legal_play((RED, 5)) is True, "Red card should match red top card"
    assert game.is_legal_play((YELLOW, 3)) is False, (
        "Yellow card should not match red top card"
    )
...
def test_legal_play_value_match():
    ...
    game = create_game_with_state(
        H_1=[(RED, 5), (YELLOW, 5)], H_2=[], top_card=(GREEN, 5), deck=[]
    )
...
def test_legal_play_wild_always_legal():
    ...
    game = create_game_with_state(
        H_1=[(BLACK, WILD), (BLACK, WILD_DRAW_4)], H_2=[], top_card=(RED, 7), deck=[]
    )

    assert game.is_legal_play((BLACK, WILD)) is True, "Wild should always be legal"
    assert game.is_legal_play((BLACK, WILD_DRAW_4)) is True, (
        "Wild Draw 4 should always be legal"
    )
...
def test_legal_play_illegal_rejected():
    ...
    game = create_game_with_state(
        H_1=[(YELLOW, 3), (GREEN, 8)], H_2=[], top_card=(RED, 7), deck=[]
    )

    assert game.is_legal_play((YELLOW, 3)) is False, (
        "Different color and value should be illegal"
    )
    assert game.is_legal_play((GREEN, 8)) is False, (
        "Different color and value should be illegal"
    )
...
def test_legal_play_wild_color_handling():
    ...
    game = create_game_with_state(
        H_1=[(RED, 5)], H_2=[], top_card=(BLACK, WILD), deck=[]
    )
    game.current_color = BLUE

    assert game.is_legal_play((RED, 5)) is False, (
        "Should not match when current_color is BLUE"
    )

    game.current_color = RED
    assert game.is_legal_play((RED, 5)) is True, (
        "Should match when current_color is RED"
    )
...
def test_get_legal_actions_no_legal_plays():
    ...
    game = create_game_with_state(
        H_1=[(YELLOW, 3), (GREEN, 8)], H_2=[], top_card=(RED, 7), deck=[(BLUE, 1)]
    )
...
def test_special_card_skip():
    ...
    game = create_game_with_state(H_1=[(RED, SKIP)], H_2=[], top_card=(RED, 5), deck=[])
...
def test_special_card_reverse():
    ...
    game = create_game_with_state(
        H_1=[(RED, REVERSE)], H_2=[], top_card=(RED, 5), deck=[]
    )
    ...
    assert result is True
    assert game.player_plays_again is True, (
        "Player should play again after Reverse (2-player rule)"
    )
...
def test_special_card_wild():
    ...
    game = create_game_with_state(
        H_1=[(BLACK, WILD)], H_2=[], top_card=(RED, 5), deck=[]
    )
...
def test_draw_rules_no_legal_plays():
    ...
    game = create_game_with_state(
        H_1=[(YELLOW, 3)], H_2=[], top_card=(RED, 7), deck=[(BLUE, 1)]
    )
...
def test_draw_action_adds_cards():
    ...
    game = create_game_with_state(
        H_1=[(YELLOW, 3)], H_2=[], top_card=(RED, 7), deck=[(BLUE, 1), (GREEN, 2)]
    )
...
def test_draw_pending_cards():
    ...
    game = create_game_with_state(
        H_1=[], H_2=[(YELLOW, 1)], top_card=(RED, DRAW_2), deck=[(BLUE, 1), (GREEN, 2)]
    )
    game.draw_pending = 2
...
def test_game_end_player_has_zero_cards():
    ...
    game = create_game_with_state(H_1=[(RED, 5)], H_2=[], top_card=(RED, 7), deck=[])
...
def test_game_end_no_actions_after_game_over():
    ...
    game = create_game_with_state(H_1=[], H_2=[(YELLOW, 1)], top_card=(RED, 7), deck=[])
    game.G_o = "GameOver"
...
def test_draw_after_reshuffle():
    ...
    assert result is True
    assert len(game.H_1) == initial_hand_size + 1, (
        "Should have drawn a card after reshuffle"
    )
...
def test_action_execution_invalid_illegal_play():
    ...
    game = create_game_with_state(H_1=[(YELLOW, 3)], H_2=[], top_card=(RED, 7), deck=[])
...
def test_action_execution_player_2():
    ...
    game = create_game_with_state(H_1=[], H_2=[(RED, 5)], top_card=(RED, 7), deck=[])
