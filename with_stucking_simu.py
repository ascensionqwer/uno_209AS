...
class Uno_AI:
    ...
    def __init__(self, player_id: int = 1, num_samples: int = 500, lookahead: int = 2, stuck_weight: int = 6):
        """
    ...
    #
        hand_term = len(opp_hand) - len(ai_hand)

        # 一點點 mobility heuristic
        temp_game = Uno(H_1=list(H_1), H_2=list(H_2), D_g=list(D_g), P=list(P))
        temp_game.create_S()
        opp_actions = temp_game.get_legal_actions(3 - self.player_id)
        opp_stuck = (len(opp_actions) == 1 and opp_actions[0].is_draw())
        small_stuck_bonus = 8.0 if opp_stuck else 0.0

        my_actions = temp_game.get_legal_actions(self.player_id)
        my_stuck = (len(my_actions) == 1 and my_actions[0].is_draw())
    ...
    def simulate_action(self, state: State, action: Action, player: int) -> State:
        ...
        if action.is_play():
            ...
            n = action.n
            for _ in range(min(n, len(D_g))):
                card = D_g.pop()   # same as execute_action draw from topn card
                hand.append(card)
    ...
    def expectiminimax(self, state: State, depth: int, player: int) -> float:
        ...
        # Determine if maximizing or minimizing 輪到自己就max 輪到對手時min
    ...
    def choose_action(self) -> Optional[Action]:
        ...
        for action in actions:
            ...
            for _ in range(self.num_samples):
                ...
                H_1, H_2, D_g, P, P_t, G_o = next_state
                temp_game = Uno(H_1=list(H_1),
                                H_2=list(H_2),
                                D_g=list(D_g),
                                P=list(P))
                temp_game.create_S()
                opp_id = 3 - self.player_id
                opp_actions = temp_game.get_legal_actions(opp_id)

                # stuck
                if len(opp_actions) == 1 and opp_actions[0].is_draw():
                    stuck_count += 1

            avg_value = total_value / self.num_samples
            p_stuck = stuck_count / self.num_samples
            score = avg_value + self.stuck_weight * p_stuck

            action_values[id(action)] = (action, score)
