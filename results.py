...
class ResultsAnalyzer:
    ...
    def _print_matchup_results(self, logs: List[Dict[str, Any]]):
        ...
        for log in logs:
            ...
            # Handle variant-based results
            if matchup == "all_matchups_with_variants":
                self._print_variant_results(log)
                return
    ...
    def _print_variant_results(self, log: Dict[str, Any]):
        """Print results for variant-based simulations."""
        ...
        for param_path, variants in param_groups.items():
            ...
            for variant in variants:
                ...
                print(
                    f"  {variation_type.title()} {change_pct:.0f}%: {original_val} -> {variant_val}"
                )

                if results:
                    print(
                        f"    Particle vs Naive Win Rate: {results.get('pvn_win_rate', 'N/A'):.2%}"
                    )
                    print(
                        f"    Particle vs Particle (Same) Win Rate: {results.get('pvp_same_win_rate', 'N/A'):.2%}"
                    )
                    print(
                        f"    Particle vs Particle (Mixed) Win Rate: {results.get('pvp_mixed_win_rate', 'N/A'):.2%}"
                    )
                    print(
                        f"    Avg Decision Time: {results.get('avg_decision_time', 'N/A'):.6f}s"
                    )
                    print(
                        f"    Avg Cache Size: {results.get('avg_cache_size', 'N/A'):.1f}"
                    )
    ...
    def _extract_variant_results(
        self, log: Dict[str, Any], variant_key: str
    ) -> Dict[str, Any]:
        """Extract key metrics for a specific variant."""
        player_wins = log.get("player_wins", {})

        # Extract data from the nested structure
        variant_data = player_wins.get(variant_key, {})

        # Get win rates from different matchup types
        pvn_win_rate = 0
        pvp_same_win_rate = 0
        pvp_mixed_win_rate = 0

        # Naive vs Particle Policy (player1 is naive, player2 is particle)
        naive_vs_particle = variant_data.get("naive_vs_particle_policy", {})
        if naive_vs_particle:
            win_rates = naive_vs_particle.get("win_rates", {})
            pvn_win_rate = win_rates.get("player2", 0)  # particle policy is player2

        # Particle vs Particle (same config)
        particle_vs_particle = variant_data.get(
            "particle_policy_vs_particle_policy", {}
        )
        if particle_vs_particle:
            win_rates = particle_vs_particle.get("win_rates", {})
            pvp_same_win_rate = win_rates.get(
                "player1", 0
            )  # player1 represents particle policy

        # For mixed config, we'll use the same as same config since mixed isn't in current data
        pvp_mixed_win_rate = pvp_same_win_rate

        # Extract decision times (average of particle policy players)
        avg_decision_time = 0
        decision_times = []

        if naive_vs_particle:
            p2_time = naive_vs_particle.get("avg_decision_times", {}).get("player2", 0)
            if p2_time > 0:
                decision_times.append(p2_time)

        if particle_vs_particle:
            p1_time = particle_vs_particle.get("avg_decision_times", {}).get(
                "player1", 0
            )
            p2_time = particle_vs_particle.get("avg_decision_times", {}).get(
                "player2", 0
            )
            if p1_time > 0:
                decision_times.append(p1_time)
            if p2_time > 0:
                decision_times.append(p2_time)
        ...
        # Extract cache sizes (average of particle policy players)
        avg_cache_size = 0
        cache_sizes = []

        if naive_vs_particle:
            p2_cache = naive_vs_particle.get("cache_stats", {}).get("player2", 0)
            if p2_cache and p2_cache > 0:
                cache_sizes.append(p2_cache)

        if particle_vs_particle:
            p1_cache = particle_vs_particle.get("cache_stats", {}).get("player1", 0)
            p2_cache = particle_vs_particle.get("cache_stats", {}).get("player2", 0)
            if p1_cache and p1_cache > 0:
                cache_sizes.append(p1_cache)
            if p2_cache and p2_cache > 0:
                cache_sizes.append(p2_cache)
    ...
    def print_parameter_impact(self):
        ...
        for param_path, impacts in param_impacts.items():
            ...
            for variation_type, results_list in impacts.items():
                ...
                avg_win_rate = sum(r["pvn_win_rate"] for r in results_list) / len(
                    results_list
                )
                avg_decision_time = sum(
                    r["avg_decision_time"] for r in results_list
                ) / len(results_list)
                avg_cache_size = sum(r["avg_cache_size"] for r in results_list) / len(
                    results_list
                )
    ...
    def _aggregate_visualization_data(
        self, sensitivity_logs: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for log in sensitivity_logs:
            ...
            for variant_key, variant_info in parameter_variants.items():
                ...
                param_data[param_path][variation_type]["win_rates"].append(
                    results["pvn_win_rate"]
                )
                param_data[param_path][variation_type]["decision_times"].append(
                    results["avg_decision_time"]
                )
                param_data[param_path][variation_type]["cache_sizes"].append(
                    results["avg_cache_size"]
                )
    ...
    def _plot_parameter_impact(self, param_data: Dict[str, Any], output_dir: str):
        ...
        ax1 = axes[0, 0]
        increase_wins = [
            np.mean(param_data[p]["increase"]["win_rates"])
            if param_data[p]["increase"]["win_rates"]
            else 0
            for p in params
        ]
        decrease_wins = [
            np.mean(param_data[p]["decrease"]["win_rates"])
            if param_data[p]["decrease"]["win_rates"]
            else 0
            for p in params
        ]
        ...
        ax2 = axes[0, 1]
        increase_times = [
            np.mean(param_data[p]["increase"]["decision_times"])
            if param_data[p]["increase"]["decision_times"]
            else 0
            for p in params
        ]
        decrease_times = [
            np.mean(param_data[p]["decrease"]["decision_times"])
            if param_data[p]["decrease"]["decision_times"]
            else 0
            for p in params
        ]
        ...
        ax3 = axes[1, 0]
        increase_cache = [
            np.mean(param_data[p]["increase"]["cache_sizes"])
            if param_data[p]["increase"]["cache_sizes"]
            else 0
            for p in params
        ]
        decrease_cache = [
            np.mean(param_data[p]["decrease"]["cache_sizes"])
            if param_data[p]["decrease"]["cache_sizes"]
            else 0
            for p in params
        ]
        ...
        norm_increase_wins = [
            (w - min(increase_wins + decrease_wins))
            / (max(increase_wins + decrease_wins) - min(increase_wins + decrease_wins))
            if max(increase_wins + decrease_wins) != min(increase_wins + decrease_wins)
            else 0
            for w in increase_wins
        ]
        norm_decrease_wins = [
            (w - min(increase_wins + decrease_wins))
            / (max(increase_wins + decrease_wins) - min(increase_wins + decrease_wins))
        ...
        plt.tight_layout()
        plt.savefig(
            f"{output_dir}/parameter_impact_analysis.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_win_rate_comparison(self, param_data: Dict[str, Any], output_dir: str):
        ...
        colors = plt.get_cmap("tab10")(np.linspace(0, 1, len(unique_params)))
        ...
        for i, param in enumerate(unique_params):
            ...
            for x, y, c_type in zip(x_vals, y_vals, c_types):
                ...
                ax1.scatter(
                    x, y, c=[param_colors[param]], marker=marker, s=100, alpha=0.7
                )
        ...
        # Add legend
        legend_elements = [
            mpatches.Patch(color=param_colors[param], label=param)
        ...
        plt.tight_layout()
        plt.savefig(
            f"{output_dir}/win_rate_comparison.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_performance_metrics(self, param_data: Dict[str, Any], output_dir: str):
            ...
            for change_type in ["increase", "decrease"]:
                ...
                if win_rates and decision_times:
                    ...
                    ax1.scatter(
                        ...
                        c=color,
                        s=50,
                        label=f"{param.split('.')[-1]} {change_type}",
                    )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{output_dir}/performance_metrics.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
