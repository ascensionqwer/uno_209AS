...
The Naive Agent is a simple agent that always plays a random legal move when possible. If a BLACK card is selected, the color chosen is the most frequent color in the player's hand (ties broken randomly). Otherwise, the Naive Agent draws a card.
...
\textbf{ALEX AND WESLY INSERT YOUR FINDINGS HERE}
...
\subsubsection{POMCP Core Algorithm}

\begin{algorithm}[htp]
\caption{POMCP Decision Making from Player 1 Perspective}\label{Alg-POMCP-P1}
\begin{algorithmic}[1]
  \Input{Current observation $O = (H_1, |H_2|, |D_g|, P, P_t, G_o)$}
  \Output{Selected action $a_t$}

  \State Generate particle belief state $B(h_t)$ consistent with $O$
  \State Initialize root node with history $h_t$
  \State Initialize statistics $T(h_t) \gets \{N(h_t)\gets 0,\; V(h_t)\gets 0,\; B(h_t)\}$

  \For{$i \gets 1$ \textbf{to} $N_{\text{sim}}$}
    \State Sample a complete game state $s \sim B(h_t)$
    \State Traverse the tree using PO-UCT selection until an unexplored node is reached
    \State Expand the tree with a new action--observation history
    \State Simulate to termination using rollout policy $\pi_{\text{rollout}}$ and obtain reward $R$
    \State Backpropagate $R$ along the traversed path and update $N(\cdot)$ and $V(\cdot)$
    \State Update belief states $B(h)$ for all encountered histories
  \EndFor

  \State Select the root action with the highest visit count
  \State \Return $a_t \gets \arg\max_a N(h_t a)$
\end{algorithmic}
\end{algorithm}
...
The rollout policy $\pi_{\text{rollout}}$ implements UNO-specific game logic:
$$
...
where $\text{LEGAL}(P_t) = \{c \in D : c(\text{COLOR}) = P_t(\text{COLOR}) \lor c(\text{VALUE}) = P_t(\text{VALUE}) \lor c(\text{COLOR}) = \text{BLACK}\}$ as defined in Section 2.4.

\subsubsection{Tree Pruning and History Management}

After executing real action $a_t$ and observing real outcome $o_t$:
\begin{enumerate}
    \item \textbf{Find Child}: Locate child node $T(h_t a_t o_t)$
    \item \textbf{Prune Tree}: Make this child the new root
    \item \textbf{Update History}: $h_{t+1} = h_t \circ (a_t, o_t)$
\end{enumerate}

All other histories become impossible and are discarded. This pruning eliminates impossible histories and maintains computational efficiency by focusing search on plausible future states. The persistent tree structure allows knowledge transfer between decisions while ensuring consistency with observed game dynamics.

\subsubsection{Particle Filter Integration}

Extending Section 4's belief state approximation with POMCP-specific updates:
...
\begin{itemize}
    \item $H_2^{(i)}$: Opponent's hand (7 cards)
    \item $D_g^{(i)}$: Deck composition (remaining cards)
\end{itemize}

POMCP-specific belief updates include:
\begin{itemize}
    \item \textbf{Start State Sampling}: $s \sim B(h_t)$ per simulation (vs. batch processing in Section 4)
    \item \textbf{Belief Updates During Search}: $B(h) \leftarrow B(h) \cup \{s\}$ for all encountered histories
    \item \textbf{Particle Reinvigoration}: Prevents particle deprivation during long simulations
\end{itemize}

After opponent actions, belief updates follow specific cases:
\begin{itemize}
    \item \textbf{Case 1}: Opponent plays card $z$: $B'(h) = \{s^{(i)} : z \in H_2^{(i)}\}$
    \item \textbf{Case 2}: Opponent draws card: $B'(h) = \{s^{(i)} : H_2^{(i)} \cap \text{LEGAL}(P_t) = \emptyset\}$
\end{itemize}
...
The parameters of the simulation can be adjusted to finetune the algorithm's runtime and effectiveness.


\section{Results}

\textbf{[Results section to be filled in with experimental findings, parameter analysis, and performance comparisons]}
