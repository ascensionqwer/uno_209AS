...
class Belief:
    ...
    def _compute_legal_unknown(self) -> Set[Card]:
        ...
        def _num_legal_instances(self) -> int:
        """Number of legal *instances* in L (duplicates counted)."""
        if not self.N_Pt:
            return 0
        return sum(1 for c in self.L if c in self.N_Pt)
...
def reset_prior(self):
    ...
    def _prob_no_legal(self) -> float:
        ...
        """
        L_size = len(self.L)
        N_size = self._num_legal_instances()
        k = self.h2_size - 1 if self.posterior_mode == "draw" else self.h2_size
    ...
    def entropy(self) -> float:
        """
        Approximate entropy of the belief state (in bits).

        - Prior (no extra constraints): H ≈ log₂(C(|L|, k)) where L is the unknown *instance* list.
        - Draw-mode (opponent had no legal play, then drew 1 unknown card):
            We model this as:
              1) H_old is a uniform (k-1)-subset of the non-legal instance pool V = L \ N(P_t)
              2) drawn is uniform from the remaining instances L \ H_old

            The number of (H_old, drawn) configurations is:
                C(|V|, k-1) · (|L| - (k-1))

            We report H ≈ log₂(that).
        """
        try:
            if self.posterior_mode == "draw":
                non_legal = [c for c in self.L if c not in self.N_Pt]
                ...
                if k_old < 0 or V_size < k_old:
                    return 0.0
                remaining = L_size - k_old
                if remaining <= 0:
                    return 0.0

                n_configs = math.comb(V_size, k_old) * remaining
            ...
            # Very large values or comb overflow: coarse fallback
            return float(len(self.L)) * 0.5
    ...
    def __repr__(self):
        ...
        return (
            ...
            f"|L|={len(self.L)}, |N(P_t)|={self._num_legal_instances()}, "
