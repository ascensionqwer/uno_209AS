...
class ConfigVariator:
    ...
    def generate_variants(self, percentage: float = 0.25) -> List[Tuple[Dict, List[ParameterVariant]]]:
        variants = []
        
        # Get all numeric parameters from particle_policy section
        numeric_params = self._get_numeric_parameters(self.base_config['particle_policy'])
        ...
        for param_path, value in numeric_params.items():
            ...
            self._set_nested_value(increased_config, f'particle_policy.{param_path}', increased_value)
            ...
            self._set_nested_value(decreased_config, f'particle_policy.{param_path}', decreased_value)
    ...
    def _get_numeric_parameters(self, config_section: Dict, prefix: str = "") -> Dict[str, float]:
        numeric_params = {}
    ...
    def _apply_variation(self, value: float, percentage: float, variation_type: str) -> float:
